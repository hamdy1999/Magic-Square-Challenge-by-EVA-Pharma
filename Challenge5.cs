...
namespace EvaInternshipChallenge
{
    class Challenge5
    {
        ...
        * In order to obtain the 3 smallest sums we do the following in a loop
        ...
        - with sum = 3 * x, the sum is directly proportional to x, so, we continue inrementing x 
          untill we find the second and third solution
        ...
        private static void Operation(int[,] Box)
        {
            ...
            do
            {
                ...
                if (IsMagicSquare(Box) && CheckDuplicates(x, a, b))
                {
                    count++;
                    Console.WriteLine($"Square correspond to {count}st small of sum:");
                    PrintBox(Box);
                    Console.WriteLine("\nsum of : {0}\n", 3 * Box[1, 1]);
                }
                else continue;
